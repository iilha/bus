'use strict';
const STATIC_CACHE = 'bus-static-v1';
const API_CACHE = 'bus-api-v1';
const STATIC_ASSETS = ['/','/index.html','/js/common.js','/js/bus.js','/js/bottom-sheet.js','/manifest.webapp'];
...
self.addEventListener('activate', (event) => {
  event.waitUntil(caches.keys().then(names => Promise.all(
    names.filter(n => n.startsWith('bus-') && n !== STATIC_CACHE && n !== API_CACHE).map(n => caches.delete(n))
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.hostname.includes('tdx.transportdata.tw')) {
    event.respondWith(fetch(event.request).then(r => {
      if (r.ok) { const c = r.clone(); caches.open(API_CACHE).then(cache => cache.put(event.request, c)); }
      return r;
